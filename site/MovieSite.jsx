        ...
        <>
            ...
            <main className="main-film-cont">
                <div className="image-btn-wrapper">
                    <img src={movie.thumbnail.regular.large} alt={`${ movie.category } image`} />
                </div>
                <div className="film-info-wrapper">
                    <h1 className="film-title">{movie.title}</h1>
                    <p className="film-info">
                        {movie.year} &bull; {"  "} <FontAwesomeIcon icon={movie.category === "Movie" ? faFilm : faTv}/> {"  "} {movie.category} &bull; {movie.rating}</p>
                    <p className="film-desc">{movie.description}</p>
                    <button className="bookmark-btn" onClick={() => changeBookmark(movie.id)} data-marked={movie.isBookmarked ? "true" : "false"}>
